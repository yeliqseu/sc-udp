...
void TcpBbr::CongControl (Ptr<TcpSocketState> tcb,const TcpRateOps::TcpRateConnection &rc,
    ...
    if (m_minRtt.GetMilliSeconds() >= 601) {
    std::cout
        << "[ "<< Simulator::Now ().GetSeconds () << " (s) ]"
        << "[BbrState] BandWidth: " << BbrMaxBandwidth().GetBitRate() << " min-RTT : " << m_minRtt.GetMilliSeconds () 
        << " instant-RTT: " << tcb->m_lastRtt.Get ().GetMilliSeconds ()
        << " Current cWnd: " << tcb->m_cWnd << " (bytes)" 
        << " bytesInFlight: " << tcb->m_bytesInFlight
        << " bytesLost: " << m_bytesLost
        << " pacingRate: " << tcb->m_pacingRate
        << " pacingGain: " << m_pacingGain 
        << " BBR-mode: " << ModeToString(m_mode)
        << " rs.m_priorDelivered: " << rs.m_priorDelivered
        << " m_nextRttDelivered: " << m_nextRttDelivered
        << " rs.deliveryRate: " << rs.m_deliveryRate.GetBitRate ()
        << " rc.m_delivered: " << rc.m_delivered
        << std::endl;
    }
    /*
    std::cout
        << "[ "<< Simulator::Now ().GetSeconds () << " (s) ]"
        << "[BbrState2] rs.m_priorDelivered: " << rs.m_priorDelivered << " m_nextRttDelivered: " << m_nextRttDelivered
        << std::endl;
    */
}
...
void TcpBbr::UpdateBandwidth(Ptr<TcpSocketState> tcb,const TcpRateOps::TcpRateConnection &rc,const TcpRateOps::TcpRateSample &rs){
    ...
    if(rs.m_priorDelivered>=m_nextRttDelivered){
        std::cout << "At " << Simulator::Now ().GetSeconds ()
                  ...
                  << " m_nextRttDelivered: " << m_nextRttDelivered
                  << " NewRoundStart"
                  << " Current rc.m_delivered: " << rc.m_delivered
                  << std::endl;
...
void TcpBbr::CheckFullBandwidthReached(const TcpRateOps::TcpRateSample &rs){
    ...
    if (bw>=target){
        std::cout << "At " << Simulator::Now ().GetSeconds ()
                  << " previous m_fullBandwidth: " << m_fullBandwidth
                  << " new m_fullBandwidth: " << bw
                  << std::endl;
        m_fullBandwidth=bw;
        m_fullBandwidthCount=0;
        return ;
...
bool TcpBbr::SetCongestionWindowRecoveryOrRestore(Ptr<TcpSocketState> tcb,const TcpRateOps::TcpRateConnection &rc,
    ...
    */
    ...
    if(TcpSocketState::CA_RECOVERY==state&&prev_state!=TcpSocketState::CA_RECOVERY){
        ...
        std::cout << "At " << Simulator::Now ().GetSeconds () << " enters CA_RECOVERY, NewRoundStart with m_nextRttDelivered: " << rc.m_delivered << std::endl; 
