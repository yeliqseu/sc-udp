...
//                   MakeEnumChecker (CLOSED, "Closed"))
    ...
    .AddAttribute ("Timestamp", "Enable or disable Timestamp option",
                   ...
                   MakeBooleanChecker ())
    .AddAttribute ("EnableFec", "Enable FEC option",
                   BooleanValue (false),
                   MakeBooleanAccessor (&TcpSocketBase::m_fecEnabled),
                   MakeBooleanChecker ())
    .AddAttribute ("MinRto",
                          ...
                          Ptr<Ipv4Interface> incomingInterface)
{
  ...
  // FEC repair packets have seqNum same as the source packet at the front of the coding block
  // So, Do NOT treat them as invalid tcp segments
  if (!IsValidTcpSegment (tcpHeader.GetSequenceNumber (), bytesRemoved,
                          packet->GetSize () - bytesRemoved) && !tcpHeader.HasOption(TcpOption::FEC))
    {
      return;
    }
  ...
  if (tcpHeader.GetFlags () & TcpHeader::SYN)
    {
      ...
      // When seeing FEC option in SYN packets
      if (tcpHeader.HasOption (TcpOption::FEC) && m_fecEnabled)
        {
          ProcessOptionFecForSyn (tcpHeader.GetOption (TcpOption::FEC)); 
        }
      else
        {
          // TODO: 需确保收发两端都发送FEC option
          // m_fecEnabled = false;
        }
...
TcpSocketBase::DupAck (uint32_t currentDelivered)
{
  ...
  if (m_tcb->m_congState == TcpSocketState::CA_RECOVERY)
  ...
  else if (m_tcb->m_congState == TcpSocketState::CA_DISORDER)
    {
      ...
      //else if (m_fecEnabled && m_fecNeedRetrans)
      //  {
      //    EnterRecovery (currentDelivered);
      //    NS_ASSERT (m_tcb->m_congState == TcpSocketState::CA_RECOVERY);
      //    m_fecNeedRetrans = false;
      //  }
      else
        {
...
TcpSocketBase::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  // Check and process FEC option
  if (m_fecEnabled)
    {
      std::cout << "At " << Simulator::Now ().GetSeconds () 
                << " node " << m_node->GetId() 
                << " received ACK which acking " << tcpHeader.GetAckNumber () 
                << " current RTO: " << m_rto.Get ().GetSeconds ()
                << std::endl;
    }
  if (tcpHeader.HasOption(TcpOption::FEC))
    {
      std::cout << "At " << Simulator::Now ().GetSeconds () 
                << " node " << m_node->GetId() 
                << " processes FEC option which acking " << tcpHeader.GetAckNumber () 
                << " current RTO: " << m_rto.Get ().GetSeconds ()
                << std::endl;
      ProcessOptionFecForAck (tcpHeader.GetOption (TcpOption::FEC));
      Ptr<const TcpOptionFEC> f = DynamicCast<const TcpOptionFEC> (tcpHeader.GetOption (TcpOption::FEC));
      // if ((f->GetFlag () == TcpOptionFEC::R_FAIL || m_dupAckCount==GetRetxThresh()) && m_tcb->m_congState != TcpSocketState::CA_RECOVERY)
      if (f->GetFlag () == TcpOptionFEC::R_FAIL)
        {
          // EnterRecovery(0);
          // DoRetransmit ();
          // return;
          m_fecNeedRetrans = true;
          m_dupAckCount = GetRetxThresh() - 1;  // trigger retransmission asap
        }
    }
...
TcpSocketBase::SendEmptyPacket (uint8_t flags)
{
  ...
  if (flags & TcpHeader::ACK)
    { // If sending an ACK, cancel the delay ACK as well
      ...
      if (m_fecProcessed)
        {
          AddOptionFecForAck (header);
          m_fecProcessed = false;     // reset to false so that the fec state is not feedback for multiple times
        }
    }
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  // 判断是否是时候发送FEC修复分组
  if (m_fecEnabled && m_fecRatio == m_fecCounter)
    {
      uint32_t sz = SendFecPacket (SequenceNumber32(m_fecHeadSeqNum), m_fecCounter);
      // m_fecHeadSeqNum += m_fecCounter * m_tcb->m_segmentSize;
      m_fecCounter = 0;  // reset fec counter
      if (m_fecNewRatio != 0)   // update fecRatio if it has been changed
        {
          m_fecRatio = m_fecNewRatio;
          m_fecNewRatio = 0;
          SetRetxThresh (m_fecRatio);
          // Reset m_dupAckCount (ugly, but have no choice)
          if (m_dupAckCount >= GetRetxThresh()) 
            {
              m_dupAckCount = GetRetxThresh() - 1;
            }
          std::cout << "New m_fecRatio: " << m_fecRatio << " and set RextThresh to " << GetRetxThresh () << std::endl;
        }
      return 0;  // return 0 so that pointers in the txBuffer is not affected 
    }
  ...
  // Add fec UNCODED option and count packets in the FEC block
  if (m_fecEnabled)
    {
      if (m_fecLastFresh == 0 || m_fecLastFresh + m_tcb->m_segmentSize == seq.GetValue() ) // a packet never sent before is to be sent
        {
          if (m_fecCounter == 0)
            {
              m_fecHeadSeqNum = seq.GetValue ();
              std::cout << "At " << Simulator::Now ().GetSeconds () << " new fecHeadSeqNum = " << m_fecHeadSeqNum << std::endl;
            }
          m_fecCounter++;
          m_fecLastFresh = seq.GetValue ();
          std::cout << "At " << Simulator::Now ().GetSeconds () 
                    << " new packet seq. " << m_fecLastFresh 
                    << " is sent. fecCounter: " << m_fecCounter 
                    << std::endl;
        } 
      else  // retransmission
        {
          std::cout << "At " << Simulator::Now ().GetSeconds () << " packet seq " << seq.GetValue() 
                    << " is sent (retransmission?), but m_fecLastFresh: " << m_fecLastFresh
                    << std::endl;
        }
    }
  if (m_fecEnabled)
    {
      ...
      option->SetFlag (TcpOptionFEC::UNCODED);
      option->SetRange (0);;
      header.AppendOption (option);
      std::cout << "At " << Simulator::Now ().GetSeconds () << " node " << m_node->GetId ()
                << " send data packet " << seq << " of size " << sz
                << " fecCounter: " << m_fecCounter
                << " headSeqNum: " << m_fecHeadSeqNum
                << std::endl;
    }
...
// Send FEC repair packet
uint32_t 
TcpSocketBase::SendFecPacket (SequenceNumber32 startSeq, uint32_t len)
{
  Ptr<Packet> p = Create<Packet> (m_tcb->m_segmentSize);
  uint32_t sz = p->GetSize (); // Size of packet
  ...
  TcpHeader header;
  header.SetFlags (flags);
  header.SetSequenceNumber (startSeq);
  header.SetAckNumber (m_tcb->m_rxBuffer->NextRxSequence ());
  if (m_endPoint)
    {
      header.SetSourcePort (m_endPoint->GetLocalPort ());
  ...
  Ptr<TcpOptionFEC> option = CreateObject<TcpOptionFEC> ();
  option->SetFlag (TcpOptionFEC::ENCODED);
  uint32_t range = len * m_tcb->m_segmentSize;
  option->SetRange (range);;
  header.AppendOption (option);
  
  std::cout << "At " << Simulator::Now ().GetSeconds () << " node " << m_node->GetId ()
            << " send repair packet for range from " << startSeq << " to " << startSeq.GetValue() + (len-1)*m_tcb->m_segmentSize
            << std::endl;
...
TcpSocketBase::ReceivedData (Ptr<Packet> p, const TcpHeader& tcpHeader)
{
  ...
  // 注：如果收到的是FEC修复分组，在这里判断是否能修复成功
  // 注意，如果FEC能够修复成功，它一定修复的是expectedSeq这一segment。
  // Copy Tcp Header to overcome const restriction
  TcpHeader tcpHeader2;
  tcpHeader2.SetFlags (tcpHeader.GetFlags ());
  tcpHeader2.SetSequenceNumber (tcpHeader.GetSequenceNumber ());
  if (tcpHeader.HasOption(TcpOption::FEC)) 
    {
      
      Ptr<const TcpOptionFEC> f = DynamicCast<const TcpOptionFEC> (tcpHeader.GetOption (TcpOption::FEC));
      if (f->GetFlagString() == "UNCODED")
        {
          // m_fecProcessed = true;
          // SequenceNumber32 startSeq = tcpHeader.GetSequenceNumber ();
          // SequenceNumber32 endSeq = tcpHeader.GetSequenceNumber () + SequenceNumber32 (f->GetRange () - m_tcb->m_segmentSize);
          std::cout << "At " << Simulator::Now ().GetSeconds ()
                    << " node " << m_node->GetId ()
                    << " receives uncoded segment, seq=" << tcpHeader.GetSequenceNumber () 
                    << " pkt size=" << p->GetSize ()
                    << " type: " << f->GetFlagString ()
                    << " expectedSeq: " << expectedSeq
                    << std::endl;
        }
      if (f->GetFlagString() == "ENCODED")
        {
          m_fecProcessed = true;
          SequenceNumber32 startSeq = tcpHeader.GetSequenceNumber ();
          SequenceNumber32 endSeq = tcpHeader.GetSequenceNumber () + SequenceNumber32 (f->GetRange () - m_tcb->m_segmentSize);
          std::cout << "At " << Simulator::Now ().GetSeconds ()
                    << " node " << m_node->GetId ()
                    << " receives repair segment, seq=" << tcpHeader.GetSequenceNumber () 
                    << " pkt size=" << p->GetSize ()
                    << " type: " << f->GetFlagString ()
                    << " range from " << startSeq 
                    << " to " << endSeq
                    << " expectedSeq: " << expectedSeq
                    << std::endl;
          // Check whether FEC repair packet can help
          if (expectedSeq > endSeq)
            {
              std::cout << "All segments in the encoding window have been received, so discard the repair packet" << std::endl;
              m_fecFailed = false;
              m_fecUnrecovered = 0;
            }
          //if (startSeq < expectedSeq && expectedSeq <= endSeq)
          else
            {
              // Count missing segements in the covered window. If it is 1, FEC can recover the segment, otherwise, FEC fails
              TcpOptionSack::SackList sackList = m_tcb->m_rxBuffer->GetSackList ();
              TcpOptionSack::SackList::iterator i;
              uint32_t uncoveredSegs;
              SequenceNumber32 lostSegSeq;
              if (expectedSeq < startSeq) 
                {
                  // The whole encoding window is behind the expected segment, in this case count missing segments in the window only
                  uncoveredSegs = (endSeq - startSeq) / m_tcb->m_segmentSize + 1;
                }
              else
                {
                  lostSegSeq = expectedSeq;
                  // Expected segment is in the window, count missing segments (expected inclusive) after the expected segment
                  uncoveredSegs = (endSeq - expectedSeq) / m_tcb->m_segmentSize + 1;
                }
              for (i = sackList.begin (); i != sackList.end (); ++i)
                {
                  NS_ASSERT (i->first > expectedSeq);
                  SequenceNumber32 leftEdge = i->first;
                  SequenceNumber32 rightEdge = i->second;
                  if (i->second < startSeq) continue;   // sack-block is in front of the FEC block (expected segment is further in front of the FEC block)
                  if (i->first < startSeq) leftEdge = startSeq;
                  if (i->second > endSeq + m_tcb->m_segmentSize) rightEdge = endSeq + m_tcb->m_segmentSize;
                  uint32_t nSeg = (rightEdge - leftEdge) / m_tcb->m_segmentSize;
                  std::cout << "SackBlock leftEdge: " << i->first << " rightEdge: " << i->second << " Sack-Covered-Segments: " << nSeg << std::endl;
                  if (rightEdge < endSeq + m_tcb->m_segmentSize)
                    {
                      lostSegSeq = rightEdge;     // at least one segment is lost in this block, record its sequence number
                    }
                  uncoveredSegs -= nSeg;
                }
              std::cout << " Remaining covered segments: " << uncoveredSegs << std::endl;
              if (uncoveredSegs > 1) {
                m_fecFailed = true;
                m_fecUnrecovered = uncoveredSegs * m_tcb->m_segmentSize;
                return;
              } else {
                if (uncoveredSegs == 1) 
                  {
                    // One lost segment in the block is recovered, add it to rxBuffer, which will update SACk list
                    TcpHeader tcpHeader3;
                    tcpHeader3.SetSequenceNumber (lostSegSeq);
                    Ptr<Packet> pkt = Create<Packet> (p->GetSize());
                    if (expectedSeq < startSeq) 
                    {
                      m_tcb->m_rxBuffer->Add (pkt, tcpHeader3);  // If the block is behind the expectedSeq, update the SACK list
                    }
                    std::cout << "Exactly one segment is lost in this block, which is: " << lostSegSeq << " and it is recovered by the repair packet and add to rxBuffer." << std::endl; 
                  }
                if (expectedSeq >= startSeq && expectedSeq <=endSeq)
                  {
                    tcpHeader2.SetSequenceNumber (expectedSeq);
                    std::cout << "At " << Simulator::Now ().GetSeconds () 
                              << " FEC successfully recovered seqNum: " << expectedSeq
                              << std::endl;
                  }
                // the whole window is safe, so no failure for the current block, even though there might be segments missing in previous blocks
                m_fecFailed = false;
                m_fecUnrecovered = 0;
              }
  ...
  if (!m_tcb->m_rxBuffer->Add (p, tcpHeader2))
  ...
  if (expectedSeq < m_tcb->m_rxBuffer->NextRxSequence ())
    { // NextRxSeq advanced, we have something to send to the app
      ...
      if (m_tcb->m_rxBuffer->Finished () && (tcpHeader2.GetFlags () & TcpHeader::FIN) == 0)
        {
          DoPeerClose ();
          return;
        }
...
TcpSocketBase::ReTxTimeout ()
{
  ...
  NS_LOG_UNCOND (this << " ReTxTimeout Expired at time " << Simulator::Now ().GetSeconds () << " current RTO: " << m_rto.Get ().GetSeconds ());
...
TcpSocketBase::DoRetransmit ()
{
  ...
  //std::cout << "At " << Simulator::Now ().GetSeconds () 
  //          << " node " << m_node->GetId()
  //          << " DoRetransmit() send seq no. " << seq
  //          << " (not necessarily a true retransmission)"
  //          << std::endl;
...
void
TcpSocketBase::AddOptionFecForSyn (TcpHeader& header)
{
  NS_LOG_FUNCTION (this << header);

  Ptr<TcpOptionFEC> option = CreateObject<TcpOptionFEC> ();
  option->SetFlag (TcpOptionFEC::SYN);
  option->SetRange (0);;

  header.AppendOption (option);
  std::cout << "At " << Simulator::Now ().GetSeconds ()
            << " node " << m_node->GetId () 
            << " add option FEC for SYN, flag=" << option->GetFlagString () << " range=" << option->GetRange ()
            << std::endl;
}

void 
TcpSocketBase::ProcessOptionFecForSyn (const Ptr<const TcpOption> option)
{
  NS_LOG_FUNCTION (this << option);

  Ptr<const TcpOptionFEC> f = DynamicCast<const TcpOptionFEC> (option);
  std::cout << "At " << Simulator::Now ().GetSeconds ()
            << " node " << m_node->GetId () 
            << " sees option FEC for SYN, flag=" << f->GetFlagString () << " range=" << f->GetRange ()
            << std::endl;
  
}

void
TcpSocketBase::AddOptionFecForEncodedData (TcpHeader& header)
{

}

void
TcpSocketBase::AddOptionFecForUncodedData (TcpHeader& header)
{

}

void
TcpSocketBase::AddOptionFecForAck (TcpHeader& header)
{
  NS_LOG_FUNCTION (this << header);
  ...
  if (m_fecFailed) 
    {
      option->SetFlag (TcpOptionFEC::R_FAIL);
      option->SetRange (m_fecUnrecovered);
    }
  else
    {
      option->SetFlag (TcpOptionFEC::R_SUCCESS);
      option->SetRange (0);
    }
  ...
  std::cout << "At " << Simulator::Now ().GetSeconds ()
            << " node " << m_node->GetId () 
            << " add option FEC for ACKing " << header.GetAckNumber() << " flag= " << option->GetFlagString () << " range= " << option->GetRange ()
            << std::endl;
}

void 
TcpSocketBase::ProcessOptionFecForAck (const Ptr<const TcpOption> option)
{
  NS_LOG_FUNCTION (this << option);

  Ptr<const TcpOptionFEC> f = DynamicCast<const TcpOptionFEC> (option);
  if (f->GetFlag () == TcpOptionFEC::R_FAIL || f->GetFlag() == TcpOptionFEC::R_SUCCESS)
    {
      std::cout << "At " << Simulator::Now ().GetSeconds ()
                << " node " << m_node->GetId () 
                << " sees option FEC for ACK, flag= " << f->GetFlagString () << " range= " << f->GetRange ()
                << std::endl;
      uint32_t total = m_tcb->m_nextTxSequence.Get ().GetValue() - 1;
      m_fecCurrTxCount = total - m_fecPrevTotal;
      if (f->GetFlag () == TcpOptionFEC::R_FAIL)
        {
          m_fecRetransCount += f->GetRange ();
          m_fecCurrRtxCount += f->GetRange ();
          // use m_nextTxSequence number to approximate the total number of transmitted bytes so far
          std::cout << " Bytes need to retransmit: " << m_fecRetransCount 
                    << " | total transmitted: " << total
                    << std::endl;
        }
      if (Simulator::Now () - m_fecLastResUpdate >= 3 * m_tcb->m_minRtt)
        {
          double newResidual = (double) m_fecCurrRtxCount / (m_fecCurrTxCount - m_fecCurrRtxCount);
          double avgResidual = (m_fecPrevResidual + newResidual) / 2;
          m_fecLastResUpdate = Simulator::Now ();
          m_fecPrevResidual = newResidual;
          m_fecPrevTotal = total;
          m_fecCurrRtxCount = 0;

          m_fecResidualLoss =  (double) m_fecRetransCount / (total - m_fecRetransCount); // moving-average
          std::cout << "At " << Simulator::Now ().GetSeconds ()
                    << " new cycle residual loss: " << newResidual
                    << " update average residual loss to: " << avgResidual
                    << std::endl;
          double delta = 0.33;
          double target = 0.001;
          if (avgResidual > target)
            {
              m_fecNewRatio = (uint32_t) std::round(m_fecRatio * (1 - delta));
            }
          else
            {
              m_fecNewRatio = (uint32_t) std::round(m_fecRatio * (1 + delta));
            }
            m_fecNewRatio = std::max(uint32_t(4), m_fecNewRatio);
            m_fecNewRatio = std::min(uint32_t(256), m_fecNewRatio);
            m_fecNewRatio = m_fecRatio;   // test fixed ratio: 1:9
        }
    }
}
