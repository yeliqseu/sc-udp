...
protected:
  ...
  void AddOptionTimestamp (TcpHeader& header);
   ...
   /**
   *  \brief Add options for FEC support
   */
  void AddOptionFecForSyn (TcpHeader& header);
  void ProcessOptionFecForSyn (const Ptr<const TcpOption> option);
  void AddOptionFecForEncodedData (TcpHeader& header);
  void AddOptionFecForUncodedData (TcpHeader& header);
  void AddOptionFecForAck (TcpHeader& header);
  void ProcessOptionFecForAck (const Ptr<const TcpOption> option);
  uint32_t SendFecPacket (SequenceNumber32 startSeq, uint32_t len);
...
protected:
  ...
  // FEC support
  bool    m_fecEnabled        {false};    // fec option enabled
  uint32_t m_fecRatio         {9};        // fec ratio, i.e., block length where fec repair packet is sent after every m_fecRatio segments
  uint32_t m_fecNewRatio      {0};        // has set new ratio, to apply new ratio when old block finishes (i.e., a repair is sent)
  uint32_t m_fecCounter       {0};        // counter of uncoded packets since last fec repair packet
  uint32_t m_fecHeadSeqNum    {0};        // head segment sequence number of the current encoding block
  uint32_t m_fecLastFresh     {0};        // store the last freshly sent segment's seq number
  bool     m_fecProcessed     {false};    // set true when a fec repair packet is received and processed, reset to false after sending ACK for it
  bool     m_fecFailed        {false};    // the latest FEC repair's status (fail to recover or success)
  bool     m_fecNeedRetrans   {false};    // receiver reports R_FAIL, to enter recovery
  uint32_t m_fecUnrecovered   {0};        // Not-recovered packets that need re-transmission
  uint32_t m_fecRetransCount  {0};        // Count re-transmissions after fec fails to recover
  double   m_fecResidualLoss  {0.0};      // Residual loss (Ferlin2018TNET)
  Time     m_fecLastResUpdate {Seconds(0)}; // Timer to update residual loss value
  uint32_t m_fecCurrRtxCount  {0};
  uint32_t m_fecCurrTxCount   {0};
  double   m_fecPrevResidual  {0.0};
  uint32_t m_fecPrevTotal     {0};
